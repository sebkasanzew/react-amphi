 ...
 */
export async function exportToExcel<T extends object>(data: T[], options: ExportOptions): Promise<string> {
	const fileId = uuidv4()
 ...
 */
export function getTemporaryFilePath(fileId: string): string {
 ...
 */
export function cleanupTemporaryFile(fileId: string): void {
	const filePath = getTemporaryFilePath(fileId)
